...
/* Where the render result, the file browser and the preferences open by default.
 *
 * Android gets a single native window, and its surface belongs to that window
 * exclusively, so GHOST_SystemAndroid::createWindow refuses to make a second
 * one. A default of "New Window" cannot work there: rendering reports that the
 * window could not be opened, and a temporary editor would never appear at all.
 * A maximized area is the only mode the platform can honour, so that is what a
 * fresh preferences file starts with. */
/* Interface sizing. A tablet is held much closer than a monitor but has far
 * less room, so the factory interface is uncomfortable to read and its editor
 * borders are hard to grab with a finger. Both are ordinary preferences the
 * user can still change; only the starting point differs. */
/* NOTE: for the render enum it is _SCREEN that means "Maximized Area"; _AREA is
 * the plain "Image Editor" entry. The two names read the other way round. */
/* Stylus pressure. Android reports pressure normalised against the range the
 * digitiser declares, not the range a hand actually reaches: measured on an
 * S Pen against a Galaxy S24 Ultra, an ordinary stroke lands between 0.25 and
 * 0.55 and pressing as hard as is reasonable on a glass screen tops out at
 * 0.77. The factory threshold of 1.0 therefore cannot be reached at all, and
 * since a finger carries no tablet data and is treated as a constant 1.0, a pen
 * stroke came out weaker than a finger one no matter how hard it was pressed.
 *
 * The threshold sits well below the measured ceiling on purpose: full pressure
 * should be comfortable rather than maximal, and leaving the top of the range
 * unused costs far less than making 100% unreachable.
 *
 * The threshold is named once, outside the platform test below, because the
 * value has to reach the build machine as well as the phone. What "Reset to
 * Default Value" restores is the RNA default, and RNA is generated by makesrna,
 * a host tool: it reads the default off a copy of this header compiled for the
 * build machine, where __ANDROID__ is not defined, and bakes the number it
 * finds into the generated source. A value only the target compiler can see
 * therefore never reaches it, which is why resetting the slider used to hand
...
struct UserDef {
  ...
  char border_width = USER_BORDER_WIDTH_DEFAULT;
  ...
  float pressure_threshold_max = USER_PRESSURE_THRESHOLD_MAX_DEFAULT;
  /** Curve non-linearity parameter. */
  float pressure_softness = USER_PRESSURE_SOFTNESS_DEFAULT;
  ...
  eUserpref_RenderDisplayType render_display_type = USER_RENDER_DISPLAY_DEFAULT;
  eUserpref_TempSpaceDisplayType filebrowser_display_type = USER_TEMP_SPACE_DISPLAY_DEFAULT;
  eUserpref_TempSpaceDisplayType preferences_display_type = USER_TEMP_SPACE_DISPLAY_DEFAULT;
  char _pad18[7] = {};
